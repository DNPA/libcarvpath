...
int main(int argc, char *argv[]) {
   ...
   carvpath_entity *partition=carvpath_parse(image,"100+8",CARVPATH_OOR_TRUNCATE);
   if (partition == 0) {
      fprintf(stderr,"Problem parsing shortsize relative path '%s', errno=%d : %s\n","100+8",errno,carvpath_error_as_string(errno));
      return -errno;
   }
   if (carvpath_get_size(partition,0) != 8) {
       fprintf(stderr,"partition size =%" PRId64 " while expected size is 8\n",carvpath_get_size(partition,0));
       return 26;
   }
